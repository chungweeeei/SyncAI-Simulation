...
from syncai_common.srv import SetBatteryLevel
...
class RobotGateway:
    ...
    def __init__(self, logger: structlog.stdlib.BoundLogger, node: Node, robot_id: str):
        ...
        battery_service_name = f'/{robot_id}/set_battery_level'
        self._battery_client = node.create_client(SetBatteryLevel, battery_service_name)

        self._logger.info(
            "[RobotGateway] Clients created",
            nav_action=nav_action_name,
            charging_action=charging_action_name,
            navigate_with_alert_action=navigate_with_alert_action_name,
            battery_service=battery_service_name,
        )
    ...
    def charge(
        ...
        self._logger.info("[RobotGateway] Sending charging goal", x=x, y=y, yaw=yaw)
    ...
    def navigate_with_alert(
        ...
        self._logger.info("[RobotGateway] Sending navigate with alert goal", x=x, y=y, yaw=yaw)
    ...
    def set_battery_level(self, level: float) -> Tuple[bool, str]:
        if not self._battery_client.wait_for_service(timeout_sec=10.0):
            return False, "SetBatteryLevel service not available"

        request = SetBatteryLevel.Request()
        request.level = level

        self._logger.info("[RobotGateway] Setting battery level", level=level)

        future = self._battery_client.call_async(request)
        if not _wait_for_future(future, timeout=10.0):
            return False, "Timeout waiting for battery service response"
