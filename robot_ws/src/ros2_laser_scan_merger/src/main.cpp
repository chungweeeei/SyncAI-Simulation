...
class scanMerger : public rclcpp::Node
{
           ...
           rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true))
    {
        ...
        // Visualize the footprint filter box in RViz so leftover points near
        // the chassis can be checked against the current filter bounds. Same
        // frame as the merged cloud -> box overlays the points at the same Z.
        footprint_pub_ = this->create_publisher<geometry_msgs::msg::PolygonStamped>(
            "filter_footprint",
            rclcpp::QoS(1).transient_local());
        footprint_timer_ = this->create_wall_timer(
            std::chrono::seconds(1),
            std::bind(&scanMerger::publish_filter_footprint, this));
    ...
    void update_point_cloud_rgb(){
        ...
        float min_theta = 0;
        float max_theta = 0;
        // Startup-race guard: scan_callback1 fires this function immediately on
        // first scan_front, but scan_rear may not have landed yet -- deref of
        // laser2_ (nullptr) downstream is a segfault. Skip this tick and wait
        // until both scans have been seen at least once.
        if (show1_ && !laser1_) return;
        if (show2_ && !laser2_) return;

        if(show1_){
            for (float i = laser1_->angle_min; i <= laser1_->angle_max; i += laser1_->angle_increment){
                if (count >= static_cast<int>(laser1_->ranges.size())) break;
                float range = laser1_->ranges[count];
                if (!std::isfinite(range) || range < laser1_->range_min || range > laser1_->range_max) {
                    count++;
                    continue;
                }
                pcl::PointXYZRGB pt;
                pt = pcl::PointXYZRGB(static_cast<std::uint8_t>(laser1R_), static_cast<std::uint8_t>(laser1G_), static_cast<std::uint8_t>(laser1B_));
                ...
                pt.z = laser1ZOff_;
                if (pt.x >= footprintXMin_ && pt.x <= footprintXMax_ &&
                    pt.y >= footprintYMin_ && pt.y <= footprintYMax_) {
                    count++;
                    continue;
                }
                if (i < (laser1AngleMin_ * M_PI / 180)){
        ...
        if(show2_){
            for (float i = laser2_->angle_min; i <= laser2_->angle_max; i += laser2_->angle_increment){
                if (count >= static_cast<int>(laser2_->ranges.size())) break;
                float range = laser2_->ranges[count];
                if (!std::isfinite(range) || range < laser2_->range_min || range > laser2_->range_max) {
                    count++;
                    continue;
                }
                pcl::PointXYZRGB pt;
                pt = pcl::PointXYZRGB(static_cast<std::uint8_t>(laser2R_), static_cast<std::uint8_t>(laser2G_), static_cast<std::uint8_t>(laser2B_));
        ...
        // Use the earliest contributing scan stamp so downstream TF lookups
        // (costmap, AMCL) interpolate odom→map at the actual capture time.
        // `now()` injected wall-clock skew that showed up as phantom points
        // trailing the robot during motion.
        rclcpp::Time stamp1(laser1_->header.stamp);
        if (show2_ && laser2_) {
            rclcpp::Time stamp2(laser2_->header.stamp);
            pc2_msg_->header.stamp = (stamp2 < stamp1 ? stamp2 : stamp1);
        } else {
            pc2_msg_->header.stamp = stamp1;
        }
    ...
    void publish_filter_footprint(){
        refresh_params();
        geometry_msgs::msg::PolygonStamped msg;
        msg.header.frame_id = cloudFrameId_;
        ...
        push(footprintXMin_, footprintYMin_);
        push(footprintXMax_, footprintYMin_);
        push(footprintXMax_, footprintYMax_);
        push(footprintXMin_, footprintYMax_);
    ...
    void refresh_params(){
        ...
        // Footprint box (in merged/base_link frame): points inside are dropped
        // as robot self-hits. Defaults of 0/0/0/0 disable the filter.
        this->get_parameter_or<float>("footprintXMin", footprintXMin_, 0.0);
        this->get_parameter_or<float>("footprintXMax", footprintXMax_, 0.0);
        this->get_parameter_or<float>("footprintYMin", footprintYMin_, 0.0);
        this->get_parameter_or<float>("footprintYMax", footprintYMax_, 0.0);
    }
    ...
    float footprintXMin_, footprintXMax_, footprintYMin_, footprintYMax_;
